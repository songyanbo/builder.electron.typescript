<template>
  <div
    class="ability-charts-item-container"
    :id="id"
...
export default defineComponent({
  props: {
    statAbilityList: Array,
    mode: String
  },
  setup(props, context) {
    const id = String(_.random(0, Number.MAX_VALUE, true));

    const statAbilityList = computed(
      () => props.statAbilityList as AV.Object[]
    );
    ...
    const legend = computed(() => {
      const data: any[] = [];
      statAbilityList.value.forEach(statTarget => {
